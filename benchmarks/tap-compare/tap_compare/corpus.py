...
Parameterised query classes and a deterministic corpus of parameter
combinations. Deterministic matters three times over here: two runs have to
issue the same queries to be comparable at all, two *servers* have to be
asked the same questions for a comparison to mean anything, and the corpus
is hashed into the provenance so a result can never be quietly compared
against a different workload.

The parameters are drawn from the *generated data*, not from thin air. The
dataset generator (dataset/egernia_dataset/generate.sql) derives every value
from ``md5(seed:index:salt)``, and that hash is reimplemented here — so a
cone search can aim at a project's real pointing without querying the
database first. A corpus of coordinates chosen independently of the data
would spend its time measuring empty results, and a corpus of one fixed
coordinate would measure the page cache.

Classes marked ``egernia_only`` touch ``srcnet.*`` tables no other TAP
server carries; they never enter a cross-server comparison.
"""
...
def rnd(seed: str, index: int, salt: str) -> float:
    """bench.rnd() from dataset/egernia_dataset/generate.sql, in Python.
...
def object_position(seed: str, project_index: int) -> tuple[float, float]:
    """Where project ``project_index``'s survey field actually points.

    The generator keys the pointing on the *project* index with salts 'ra'
    and 'dec' (each product scatters within a degree of it), which is
    exactly what this recomputes.
    """
    ra = 360.0 * rnd(seed, project_index, "ra")
    dec = math.degrees(math.asin(2.0 * rnd(seed, project_index, "dec") - 1.0))
    return ra, dec
...
#: The ObsCore 1.1 mandatory columns the portable classes project. Every
#: compared server carries these; nothing egernia-specific may appear here.
OBSCORE_COLUMNS = (
...
class QueryClass:
    ...
    template: str
    stress: bool = False
    #: touches tables only egernia serves; excluded from cross-server runs
    egernia_only: bool = False
...
#: The classes any conformant ObsTAP server can answer.
PORTABLE = tuple(cls for cls, spec in CLASSES.items() if not spec.egernia_only)


def _parameters(rng: Deterministic, cls: str, seed: str, projects: int, cfg: dict) -> dict:
    gen = cfg["generation"]
    mjd_lo, mjd_hi = gen["mjd_range"]
    span = mjd_hi - mjd_lo

    # Cone centres: mostly aimed at a real survey field, some deliberately
    # not. A corpus that only ever aims at fields never measures the empty
    # path, and one that never does measures nothing but it.
    def cone_centre() -> tuple[float, float]:
        if rng._next() < 0.8:
            return object_position(seed, rng.randint(1, max(projects, 1)))
    ...
    if cls == "Q02":
        ...
        return {"project_id": f"SKAO-P{rng.randint(1, max(projects, 1)):09d}"}
    if cls in ("Q03", "Q11"):
        return {
    ...
    if cls == "Q12":
        # Never aimed at a field: this class exists to measure the cost of
...
def build(
    cfg: dict, dataset_cfg: dict, projects: int, *, portable_only: bool = False
) -> list[CorpusEntry]:
    """A deterministic corpus spanning every (selected) class.
    ...
    still comparable query-for-query. ``portable_only`` restricts the corpus
    to the classes any ObsTAP server can answer — the cross-server setting.
    """
    seed = str(dataset_cfg["generation"]["seed"])
    target = cfg["corpus"]["combinations"]
    rng = Deterministic(cfg["corpus"]["seed"])
    classes = [c for c in CLASSES if not (portable_only and CLASSES[c].egernia_only)]
    ...
    def draw(cls: str) -> bool:
        ...
        params = _parameters(rng, cls, seed, projects, dataset_cfg)
