...
log = logging.getLogger("egernia_bench.prometheus")
...
API = 'namespace="default",pod=~"egernia-tap-api-.*",container!=""'
EXEC = 'namespace="default",pod=~"egernia-tap-executor-.*",container!=""'
PG = 'namespace="default",pod=~"egernia-postgres-.*",container!=""'
# Network and filesystem series are per pod, not per container, so they need
# selectors without the container filter.
API_POD = 'namespace="default",pod=~"egernia-tap-api-.*"'
PG_POD = 'namespace="default",pod=~"egernia-postgres-.*"'
...
def _deploy(metric: str, component: str) -> str:
    return f'{metric}{{namespace="default",deployment="egernia-{component}"}}'
