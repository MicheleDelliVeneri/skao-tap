...
box:
          ...
          RPAREN ;
cast_specification:             CAST LPAREN value_expression AS cast_target RPAREN ;
cast_target:
          SMALLINT | INTEGER | BIGINT | REAL_SYM | DOUBLE PRECISION
        | CHAR ( LPAREN unsigned_decimal RPAREN )?
        | VARCHAR ( LPAREN unsigned_decimal RPAREN )?
        | TIMESTAMP
        | POINT | CIRCLE | POLYGON ;
catalog_name:                   ID ;
coalesce_expression:            COALESCE LPAREN value_expression ( COMMA value_expression )+ RPAREN ;
// centroid:                       CENTROID LPAREN geometry_value_expression RPAREN ;
// character_representation:       nonquote_character ;// | SQ SQ ;
char_function:                  ( LOWER | UPPER ) LPAREN character_value_expression RPAREN ;
...
geometry_value_expression:      box | circle | point | polygon | user_defined_function | column_reference ; //| centroid | region
